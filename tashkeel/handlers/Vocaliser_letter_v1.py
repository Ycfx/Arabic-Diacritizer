...
class Vocaliser:
    ...
    def AbsoluteDiscountingSmoothing(self,w2,w1,d,v):
        ...
        count_w2 = hdb.SelectOne('words','word="'+w2+'"',attribute='freq')
        count_w1_w2 = hdb.SelectOne('grams2','grams="'+w1+' '+w2+'"',attribute='freq')
        count_w_w2 = hdb.execute('SELECT SUM(freq) FROM grams2 WHERE grams LIKE "'+w1+' %"')
        ...
        if count_w_w2 != None: count_w_w2 = count_w_w2[0][0]
        else : count_w_w2 = 0
        """
        pprint(count_w1)
        pprint(count_w2)
        ...
        pprint(count_w_w2)
        """
        lamda = d*count_w_w2/count_w1
        return ((count_w1_w2-d)/count_w1)+(lamda*count_w2/v)

        #pprint('SELECT SUM(freq) FROM grams2 WHERE grams LIKE "'+w1+' %"')
        #lamda = d*count_w_w2/count_w1
        #return  ((count_w1_w2-d)/count_w1)+(lamda*count_w2/v)
